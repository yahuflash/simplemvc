package simplemvc.action
{
	...
	public final class ActionManager
	{
		...
		public function ActionManager(){
			...
			SimpleEventDispatcher.sharedSimpleDispatcher().addEventListener(Module.MODULE_DISPOSED,function(args:Object):void{
		...
		internal function run(action:Action):void{
			if (action is NotifyAction){
				...
				//如果是拉取动作，执行，在完成后回收
				action.addEventListener(SimpleCommand.COMPLETE,function():void{
					remove(action);
					action.release();
				});
				action.execute();
